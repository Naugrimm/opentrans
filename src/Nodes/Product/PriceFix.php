 ...
 * @method self setAllowOrChargesFix(null|array<string, mixed>|\Naugrim\OpenTrans\Nodes\AllowOrChargesFix $allowOrChargesFix)
 * @method \Naugrim\OpenTrans\Nodes\AllowOrChargesFix|null getAllowOrChargesFix()
 * @method self setTax(\Naugrim\OpenTrans\Nodes\Tax\DetailsFix[]|array<string, mixed> $tax)
 * @method \Naugrim\OpenTrans\Nodes\Tax\DetailsFix[] getTax()
...
class PriceFix implements NodeInterface
{
    ...
    {
        $taxes = $this->getTax();
        Assert::allIsInstanceOf($taxes, DetailsFix::class);
    ...
    public function finalPriceIncludingTaxes(): float
    {
        ...
        foreach ($this->getTaxSequence() as $tax) {
            ...
            if ($tax->getAmount() !== null) {
                $finalPrice += (float) $tax->getAmount();
            } elseif ($tax->getTax() !== null) {
                $finalPrice += round($finalPrice * ((float) $tax->getTax()), 2);
            }
        }
